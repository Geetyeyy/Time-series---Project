...
par(mfrow=c(1,1))
plot(airtsdiff,type='o', ylab ="Temperature Anomalies", main="Time series plot of the first
difference of Temperature series.")
```

- The first differenced time series of temperature anomalies from 2003 to 2023 is displayed in the plot. The series shows a more stable mean after differencing, suggesting that the underlying tendency has been eliminated. Overall variability is more stable, although there are still bigger changes during specific periods, such around 2020. The series becomes stationary and the variance is stabilised with the aid of this transformation. The data has been successfully detrended by the first differencing, which improves its suitability for ARIMA modelling and forecasting. To verify that the differenced series is stationar, additional tests can be performed, such as the Augmented Dickey-Fuller (ADF) test.
...
par(mfrow=c(1,1))
plot(airtsdiff2,type='o', ylab ="Temperature Anomalies", main="Time series plot of the second difference of Temperature series.")
```
...
- The time series' stationarity was considerably enhanced by the second differencing, as shown by the outcomes of the Augmented Dickey-Fuller (ADF) test. We are able to reject the null hypothesis of non-stationarity based on the Dickey-Fuller statistic of -9.8887 with a p-value of 0.01 obtained from the ADF test. The series is now stationary, as this verifies.
- With most values falling within the confidence intervals, the ACF plot displays decreased autocorrelation and less reliance on historical data. Less notable spikes are shown in the PACF plot, which has a distinct cutoff at lag 1 and suggests a more straightforward ARIMA model structure. An MA(1) and maybe an AR(1) term can be included to represent the inverse link between the current value and the negative spike observed at lag 1 in both plots.
